...
import com.neoaplicacoes.customer_api.mapper.CustomerMapper;
...
import com.neoaplicacoes.customer_api.repository.CustomerRepository;
import com.neoaplicacoes.customer_api.service.impl.CustomerServiceImpl;
...
/**
 * Unit tests for CustomerServiceImpl using Mockito.
 * The repository and mapper are mocked to isolate the service logic.
 */
@ActiveProfiles("test")
...
class CustomerServiceImplTest {
    ...
    void setUp() {
        ...
        request = new CustomerRequestDTO(
                "John Doe", "john@doe.com", "12345678901",
                "11999999999", LocalDate.of(1990, 1, 1), addrReq
        );

        response = new CustomerResponseDTO(
                1L, "John Doe", "john@doe.com", "12345678901",
                "11999999999", LocalDate.of(1990, 1, 1),  // birthDate
                34, // age (will be ignored in assertions that do not care)
                new AddressResponseDTO(1L,"12345678", "100", "Apt 10", "Main St",
                        "Centro", "São Paulo", "SP")
        );

        // Generic stub: whenever mapper.toDto(any Customer) is called, build a DTO mirroring the entity.
        lenient().when(mapper.toDto(any(Customer.class))).thenAnswer(inv -> {
            Customer c = inv.getArgument(0);
            Address a = c.getAddress();
            AddressResponseDTO aDto = a == null ? null :
                    new AddressResponseDTO(  a.getId(),  a.getCep(), a.getNumber(), a.getComplement(),
                            a.getStreet(), a.getNeighborhood(), a.getCity(), a.getState());
            return new CustomerResponseDTO(c.getId(), c.getName(), c.getEmail(), c.getCpf(),
                    c.getPhone(), c.getBirthDate(), c.getAge(), aDto);
        });
        lenient().when(mapper.toDtoList(anyList())).thenAnswer(inv -> {
    ...
    void update_success() {
        ...
        AddressRequestDTO addrUpdate = new AddressRequestDTO(
                "87654321", null, null, "New St", "Bairro", "Rio", "RJ"
        );
        CustomerRequestDTO updateReq = new CustomerRequestDTO(
                "Jane", null, null, null, null, addrUpdate
        );
    ...
    void getByCityAndNeighborhood_success() {
        when(repository.findByAddressCityIgnoreCaseAndAddressNeighborhoodIgnoreCase("São Paulo", "Centro"))
    ...
    class Pagination {
        ...
        void getByCityAndNeighborhoodPaged_success() {
            when(repository.findByAddressCityIgnoreCaseAndAddressNeighborhoodIgnoreCase("São Paulo", "Centro", pageable))
                    .thenReturn(page);
            assertThat(service.getByCityAndNeighborhoodPaged("São Paulo", "Centro", pageable).getContent()).hasSize(1);
        }
    }
}
