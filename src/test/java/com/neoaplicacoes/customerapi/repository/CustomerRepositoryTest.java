...
/**
 * Repository tests using @DataJpaTest with H2. Each query method is validated against deterministic
 * seed data.
 */
@ActiveProfiles("test")
@DataJpaTest
class CustomerRepositoryTest {
  ...
  void setUp() {
    ...
    c1 =
        customer(
            "John Doe", "john@doe.com", "12345678901", "11911111111", "São Paulo", "SP", "Centro");
    c2 =
        customer(
            "Jane Roe", "jane@roe.com", "22222222222", "11922222222", "Rio", "RJ", "Copacabana");
    c3 =
        customer(
            "Joana Silva",
            "joana@silva.com",
  ...
  void findByNameOrEmailPaged() {
    ...
    Page<Customer> page =
        repository.findByNameContainingIgnoreCaseOrEmailContainingIgnoreCase("jo", "roe", pageable);
  ...
  void findByCityAndNeighborhood() {
    assertThat(
            repository.findByAddressCityIgnoreCaseAndAddressNeighborhoodIgnoreCase(
                "São Paulo", "Centro"))
        .extracting(Customer::getId)
        .containsExactly(c1.getId());
