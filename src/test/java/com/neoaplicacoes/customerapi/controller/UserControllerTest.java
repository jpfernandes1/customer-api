...
class UserControllerTest {
  ...
  void setup() {
    ...
    sampleResponse =
        new UserResponseDTO(
            1L, "user@example.com", "ROLE_USER", true, LocalDateTime.now(), LocalDateTime.now());
  }
  ...
  void deleteByAdmin() throws Exception {
    ...
    mvc.perform(delete("/api/users/1")).andExpect(status().isNoContent());
  }
  ...
  void getAllPagedAsAdmin() throws Exception {
    ...
    when(userService.getAllPaged(any())).thenReturn(page);
  ...
  void getAllPagedAsUserForbidden() throws Exception {
    mvc.perform(get("/api/users/paged?page=0&size=10")).andExpect(status().isForbidden());
  }
  ...
  void getByRoleAsAdmin() throws Exception {
    ...
    mvc.perform(get("/api/users/search/by-role?role=ROLE_USER").accept(MediaType.APPLICATION_JSON))
        .andExpect(status().isOk())
  ...
  void getByRoleAsUserForbidden() throws Exception {
    mvc.perform(get("/api/users/search/by-role?role=ROLE_USER")).andExpect(status().isForbidden());
  }
